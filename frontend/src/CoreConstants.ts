...
export const featureFlags : Record<string,any> = JSON.parse(checkEnv("VITE_FEATURE_FLAGS") || "{}");
...
export const cognitoLogoutChainUrl : string = checkEnv("VITE_AWS_COGNITO_LOGOUT_CHAIN_URL");
export const cognitoEnvironment : string = checkEnv("VITE_AWS_COGNITO_ENVIRONMENT");
...
export const awsconfig  = {
  Auth:{
    Cognito:{
      ...
      loginWith: {
        oauth: {
          domain: nodeEnv === 'test' ? backendUrl.replace("http://","").replace(":8080",":8081") : `${cognitoDomain}.auth.${cognitoRegion}.amazoncognito.com`,
