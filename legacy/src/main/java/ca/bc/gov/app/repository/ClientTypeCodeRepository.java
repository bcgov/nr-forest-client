 ...
 *
 * <p>Provides non-blocking CRUD operations and custom queries for client type
 * codes. Utilizes Spring Data R2DBC for reactive database access.
 */
@Repository
public interface ClientTypeCodeRepository
   ...
   *
   * <p>A client type code is considered active if its expiry date is null or after
   * the given date, and its effective date is on or before the given date.
   *
   * @param activeDate the date to check for active client type codes
   * @return a {@link Flux} emitting {@link CodeNameDto} objects representing
   *     active client type codes
   */
  @Query("""
