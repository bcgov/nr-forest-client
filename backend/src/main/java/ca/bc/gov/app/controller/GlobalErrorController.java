...
/**
 * The GlobalErrorController class extends the AbstractErrorWebExceptionHandler class. It is
 * annotated with @RestControllerAdvice, @ControllerAdvice, @Slf4j, @Component, and @Order.
 *
 * @RestControllerAdvice is a convenience annotation that is itself annotated with @ControllerAdvice
 * and @ResponseBody. This annotation is used to assist with exception handling in a @Controller
 * class.
 * @ControllerAdvice is an annotation provided by Spring allowing you to handle exceptions across
 * the whole application, not just an individual controller. You can think of it as an interceptor
 * of exceptions thrown by methods annotated with @RequestMapping and similar.
 * @Slf4j is a simple facade for logging systems allowing the end-user to plug in the desired
 * logging system at deployment time.
 * @Component is an annotation that allows Spring to automatically detect our custom beans.
 * @Order is used to sort the components that Spring should load in the ApplicationContext.
 * <p>
 * The GlobalErrorController class is responsible for handling and routing all the exceptions that
...
public class GlobalErrorController extends AbstractErrorWebExceptionHandler {
  ...
  /**
   * This method is responsible for handling and rendering error responses. It takes a ServerRequest
   * and ErrorAttributes as parameters. The method first retrieves the error associated with the
   * request and logs it. If the error is an instance of ValidationException, it logs the validation
   * errors and returns a response with the status code, reason, and errors from the exception. If
   * the error is not a ValidationException, it checks if it's a ResponseStatusException. If it is,
   * it retrieves the reason and status code from the exception. If the error message is blank, it
   * sets it to an empty string. Finally, it logs the error status and message, and returns a
   * response with the status code and error message.
   *
  ...
  /**
   * This method is used to handle CSRF (Cross-Site Request Forgery) tokens in the application. It
   * is annotated with @ModelAttribute, which means its executed before any request mapping methods.
   * The method returns a Mono<Void> as the cookie will be set asynchronously behind the scene and
   * this is here only to subscribe the requester to receive the cookie.
   * <p>
   * Another important thing here is that this method will always set the csrf token on the stream.
   *
   * @param exchange The ServerWebExchange interface provides access to information about the
   *                 request and response.
   * @return Mono<Void> If the CSRF token is not present in the ServerWebExchange, it returns an
   * empty Mono. If the CSRF token is present, it adds the token to the ServerWebExchange attributes
   * and then returns a Mono<Void> indicating completion.
   */
  @ModelAttribute
