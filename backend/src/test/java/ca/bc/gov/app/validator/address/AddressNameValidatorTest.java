...
@DisplayName("Unit Tests | AddressNameValidator")
class AddressNameValidatorTest {

  private final AddressNameValidator validator = new AddressNameValidator();
  ...
  void shouldValidate(String name, String expectedMessage) {
    ...
    if (StringUtils.isNotBlank(expectedMessage)) {
      validation.expectNext(new ValidationError(
              "location.addresses[0].locationName",
              expectedMessage
          )
      );
    }
  ...
  private static Stream<Arguments> validation() {
    return
        Stream.of(
            Arguments.of(StringUtils.EMPTY, "You must enter the name of a location."),
